...
const std::map<char, double> monoisotopicMassTable = {{'A', 71.03711},
                                                      {'C', 103.00919},
                                                      {'D', 115.02694},
                                                      {'E', 129.04259},
                                                      {'F', 147.06841},
...
double calculateProteinMass(std::string protein);

int main()
{
    std::string protein = "SKADYEK";

    double proteinMass = calculateProteinMass(protein);

    std::cout << proteinMass << '\n';
...
{
    double proteinMass;
    for (char const &ch : protein)
    {
        for (auto const &c : monoisotopicMassTable)
        {
            if (ch == c.first)
            {
                proteinMass += c.second;
            }
        }
    }
